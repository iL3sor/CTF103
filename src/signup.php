...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (UserModel::check($username)) {
        print '<script>alert("Duplicate username, try again");</script>';
        print '<script>window.location.assign("/?page=signup");</script>';
    } else {
        UserModel::signup($username, $password);
        print '<script>alert("Succesfully register");</script>';
        print '<script>window.location.assign("/?page=login");</script>';
    }
}
?>
<!-- ************** HTML CODE BOUNDARY ************** -->
        ...
        <nav class="navbar navbar-dark bg-dark">
            <div class="d-flex">
                ...
                <div class="ml-auto p-2">
                    <button type="button" class="btn btn-outline-warning" onclick="login()"><strong>Already have an account ?</strong></button>
                </div>
            </div>
